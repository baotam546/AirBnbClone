...
function PhotoUploader({ addedPhotos, setAddedPhotos }) {
    ...
    function selectAsMainPhoto(e,photo) {
        e.preventDefault();
        const addedPhotosWithoutSelected = addedPhotos.filter(p => p !== photo);
        setAddedPhotos([photo, ...addedPhotosWithoutSelected]);
    }
    return (
        <>
            ...
            <div className='mt-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-10'>
                {addedPhotos.length > 0 && addedPhotos.map(link => (
                    <div className='h-32 flex relative' key={link}>
                        ...
                        <button className='absolute bottom-1 left-1 text-white cursor-pointer bg-black bg-opacity-50 p-1 rounded-full'
